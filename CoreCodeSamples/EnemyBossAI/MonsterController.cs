...
public class MonsterController : MonoBehaviour
{
    ...
    // Move the monster toward the target (player or car)
    public void WalkToAim()
    {
        Vector3 tagPosition = GameObject.FindWithTag(aimTag).transform.position;
            ...
            {
                // Move to the target position if out of attack range
    ...
    {   
        ...
        else if(HP >0)
        {
            ...
            if (isBoss)
            {
                UIManager.instance.SetBossHealth(HP,HpCapacity); // Update boss health UI
            }
        }
    ...
    // Delay after taking damage before resuming chase
    ...
    // Handle boss death and trigger the end game
